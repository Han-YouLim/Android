...
//위의 에러가 함께 발생시, DBManger에서 DB생성시 정했던 열의 이름이 변질되어 사용되지 않았는지 체크!
class PersonalList : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        layout = findViewById(R.id.personal)

        var cursor:Cursor
        cursor = sqlitedb.rawQuery("SELECT * FROM personal", null)

        var num:Int =0
        while (cursor.moveToNext()) {
            var str_name = cursor.getString(cursor.getColumnIndex("name")).toString()
            var str_gender = cursor.getString(cursor.getColumnIndex("gen")).toString()
